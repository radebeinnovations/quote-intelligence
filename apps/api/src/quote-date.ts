...
export function parseSouthAfricanDate(input: string): string {
  ...
  if (numeric) {
    ...
    return first.length === 4
      ? isoDate(Number(first), Number(second), Number(third))
      : isoDate(Number(third), Number(second), Number(first));
  }

  const words = value.match(/^(\d{1,2})(?:st|nd|rd|th)?[\s/-]+([A-Za-z]+)[,\s/-]+(\d{4})$/i);
  if (words) {
