...
function xmlText(value: unknown): string {
  ...
  if ("r" in object) return array(object.r).map((part) => xmlText(record(part).t)).join("");
  return "";
}
...
function numeric(value: CellValue, label: string): number {
...
export function parseXlsxQuoteBuffer(bytes: Uint8Array): ExtractedDocument {
  const rows = decodeWorkbook(bytes);
  const headerIndex = rows.findIndex(
    (row) => text(row[0]).toLowerCase() === "description" && text(row[1]).toLowerCase() === "qty"
  );
  ...
  for (let index = headerIndex + 1; index < rows.length; index += 1) {
    ...
    lineItems.push({
      ...
      description,
      quantity: numeric(row[1], "quantity"),
      unit: text(row[2]),
      unitRate: numeric(row[3], "unit rate"),
      lineTotal: numeric(row[4], "line total")
    });
  }
  ...
  return extractedDocumentSchema.parse({
    ...
    quote: {
      ...
      currency: "ZAR",
      vatRate: 0.15,
      taxBasis: detectTaxBasis(rows),
      ...(subtotalRow ? { subtotal: numeric(subtotalRow[4], "subtotal") } : {}),
      ...(vatRow ? { vatAmount: numeric(vatRow[4], "VAT") } : {}),
      ...(totalLabelRow ? { total: numeric(totalLabelRow[4], "total") } : {})
    },
    lineItems,
